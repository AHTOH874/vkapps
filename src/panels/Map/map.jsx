...
const Map = ({ id, go, selectedElem }) => 
{
  const [coordsRecived, setCoordsRecived] = useState(false)
  const [places, setPlaces] = useState([])
  const [myCoords, setCoords] = useState([55.75, 37.57])
  useEffect(()=> {
    VKConnect.send('VKWebAppGetGeodata').then((data)=> { 
      console.log(data);
      if (data.aviable) {
        setCoords([data.lat, data.long])
        setCoordsRecived(true)
      }
    }).catch(console.error)
    ...
    <Panel id={id} >
      ...
      <div style={mapStyle}>
        <YMaps>
          {
            coordsRecived && 
              <YMap height='calc(100vh - 93px)' width='100%' defaultState={{ center: myCoords, zoom: 9 }}>
                <ObjectManager
                  options={{
                    clusterize: true,
                    gridSize: 32,
